...
class HexBattlefieldView(QtWidgets.QGraphicsView):
    ...
    def _item_to_army(self, item: QtWidgets.QGraphicsItem | None) -> ArmyItem | None:
        while item and not isinstance(item, ArmyItem):
            item = item.parentItem()
        return item if isinstance(item, ArmyItem) else None

    def mousePressEvent(self, event: QtGui.QMouseEvent) -> None:  # type: ignore[override]
        if event.button() == QtCore.Qt.MouseButton.LeftButton:
            gitem = self.itemAt(event.position().toPoint())
            a_item = self._item_to_army(gitem)
            if a_item:
                self._drag_idx = self._tab.armies.index(a_item.army)
            else:
                item = self._item_to_cell(gitem)
                if item:
    ...
    def mouseDoubleClickEvent(self, event: QtGui.QMouseEvent) -> None:  # type: ignore[override]
        ...
        a_item = self._item_to_army(gitem)
        if a_item:
            idx = self._tab.armies.index(a_item.army)
...
class BattlefieldTab(QtWidgets.QWidget):
    ...
    def __init__(self, main_window: 'MainWindow', width: int = 16, height: int = 16) -> None:
        ...
        # Update ten times per second for smoother movement
    ...
    def _army_at(self, x: int, y: int) -> Army | None:
        for army in self.armies:
            if (
                ...
                and int(round(army.float_x)) == x
                and int(round(army.float_y)) == y
            ):
                return army
        return None

    def _drag_move(self, idx: int, x: int, y: int) -> None:
        if 0 <= idx < len(self.armies) and self.battlefield.within_bounds(float(x), float(y)):
            army = self.armies[idx]
            army.set_destination((float(x), float(y)))
