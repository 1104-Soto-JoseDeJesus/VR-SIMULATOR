...
class MultiArmySimulator:
    ...
    def step(self) -> None:
        ...
        # Resolve battles for armies targeting the same destination
        ...
        for army in self.armies:
            if army.current_troop_count > 0 and army.battle_time_remaining == 0:
                if army.destination:
                    step = step_towards((army.x, army.y), army.destination)
                    occupant = next(
                        (a for a in self.armies if a is not army and a.current_troop_count > 0 and a.x == step[0] and a.y == step[1]),
                        None,
                    )
                    if occupant:
