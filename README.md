...
hexagonal grid that fills the tab. Armies can be moved by dragging their markers to new
tiles and double‑clicking a marker opens a dedicated configuration dialog for
that battlefield army so changes do not affect the duel tab. Use Ctrl + mouse
wheel to zoom the map. If two armies occupy the same tile they immediately
