...
class HexBattlefieldView(QtWidgets.QGraphicsView):
    ...
    def _item_to_cell(self, item: QtWidgets.QGraphicsItem | None) -> HexCellItem | None:
        """Return the enclosing ``HexCellItem`` for ``item`` if any."""
        while item and not isinstance(item, HexCellItem):
            item = item.parentItem()
        return item if isinstance(item, HexCellItem) else None

    def mousePressEvent(self, event: QtGui.QMouseEvent) -> None:  # type: ignore[override]
        if event.button() == QtCore.Qt.MouseButton.LeftButton:
            item = self._item_to_cell(self.itemAt(event.position().toPoint()))
            if item:
    ...
    def mouseReleaseEvent(self, event: QtGui.QMouseEvent) -> None:  # type: ignore[override]
        if self._drag_idx is not None:
            ...
            if item:
                ...
                q, r = self._scene_to_hex(scene_pt)
                if self._tab.battlefield.within_bounds(q, r):
                    self.armyDropped.emit(self._drag_idx, q, r)
            self._drag_idx = None
    ...
    # ------------------------------------------------------------------
    def _scene_to_hex(self, pt: QtCore.QPointF) -> tuple[int, int]:
        """Convert scene coordinates to axial hex coordinates."""
        size = self._base_size
        r = int(round(pt.y() / (size * 1.5)))
        q = int(round(pt.x() / (size * math.sqrt(3)) - (r % 2) / 2))
        return q, r
...
class BattlefieldTab(QtWidgets.QWidget):
    ...
    # ------------------------------------------------------------------
    def _assign_targeting(self, army: Army, x: int, y: int) -> None:
        target = self._army_at(x, y)
        if target and target.team != army.team:
            self._set_targeting(army, target)
        else:
    ...
    def _drag_move(self, idx: int, x: int, y: int) -> None:
        if 0 <= idx < len(self.armies):
            if self.battlefield.within_bounds(x, y):
                ...
                self._assign_targeting(army, x, y)
    ...
    def _has_opposing_teams(self, armies: list[Army]) -> bool:
        teams = {a.team for a in armies}
        return len(teams) > 1
    ...
    def _check_conflicts(self) -> None:
        ...
        for armies_here in positions.values():
            while self._has_opposing_teams(armies_here):
                a1 = armies_here.pop(0)
                # find first opponent from different team
                idx = next((i for i, a in enumerate(armies_here) if a.team != a1.team), None)
                if idx is None:
                    armies_here.insert(0, a1)
                    break
                a2 = armies_here.pop(idx)
                self._set_targeting(a1, a2)
                self._resolve_battle(a1, a2)
                if a1.current_troop_count > 0 and a1.battle_time_remaining == 0:
                    armies_here.append(a1)
                if a2.current_troop_count > 0 and a2.battle_time_remaining == 0:
                    armies_here.append(a2)
        self.armies = [a for a in self.armies if a.current_troop_count > 0]

    def _tick(self) -> None:
        finished: list[Army] = []
        for army in self.armies:
            before = army.battle_time_remaining
            army.progress_battle()
            if before > 0 and army.battle_time_remaining == 0:
                if army.pending_report:
                    army.battle_reports.append(army.pending_report)
                    army.pending_report = None
                    self.reportsUpdated.emit()
                if army.current_troop_count > 0:
                    finished.append(army)
        self.armies = [a for a in self.armies if a.current_troop_count > 0]

        for army in finished:
            self._retarget(army)
            if (
                army.direct_target
                and army.direct_target.current_troop_count > 0
                and army.direct_target.battle_time_remaining == 0
                and hex_distance((army.x, army.y), (army.direct_target.x, army.direct_target.y)) <= 1
            ):
                self._resolve_battle(army, army.direct_target)
        ...
        for army in self.armies:
            if army.current_troop_count > 0 and army.battle_time_remaining == 0:
                if army.destination:
                    ...
                    if occupant and occupant is not army and occupant.team != army.team:
                        if occupant.battle_time_remaining == 0:
                            self._set_targeting(army, occupant)
                            self._resolve_battle(army, occupant)
                        else:
                            if army not in occupant.attackers:
                                occupant.attackers.append(army)
                            army.direct_target = occupant
                        continue
    ...
    def _resolve_battle(self, army1: Army, army2: Army) -> None:
        ...
        army1.pending_report = report_text
        army2.pending_report = report_text
        ...
        # Remove dead armies and retarget survivors
