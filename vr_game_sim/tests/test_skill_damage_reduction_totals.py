...
def test_soul_awakening_replaces_legacy_counter_adjust_effect():
    counter_unit = Unit(unit_type="infantry", tier=5, initial_count=100)
    defender_unit = Unit(unit_type="infantry", tier=5, initial_count=100)
    ...
    created = defender_army._create_and_add_single_effect(
        legacy_effect,
    ...
    defender_army.activate_queued_effects()
    existing = next(
        eff
        for eff in defender_army.active_effects
        if eff.name == EFFECT_NAME_SOUL_AWAKENING_COUNTER_REDUCTION
    )
    assert existing.config.get("stat_to_mod") == StatType.COUNTER_DAMAGE_ADJUST
    ...
    assert refreshed is not existing
    assert refreshed.magnitude == -0.45
    assert refreshed.config.get("stat_to_mod") == StatType.DAMAGE_TAKEN_MULTIPLIER
    assert refreshed.config.get("config_filter") == {"attack_type": "COUNTER"}
    assert refreshed.config.get("is_dispellable") is False
