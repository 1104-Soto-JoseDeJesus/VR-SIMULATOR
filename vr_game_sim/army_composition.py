...
class Army:
    ...
    def set_destination(self, dest: Tuple[float, float]):
        ...
        self.destination = (float(dest[0]), float(dest[1]))
        self.path.clear()

    def update_position(self, battlefield: "Battlefield", dt: float = 1.0):
        ...
        if battlefield.navmesh:
            ...
            if not self.path:
                ...
                self.path = battlefield.navmesh.find_path(start, dest)
            ...
            if dist <= speed:
                ...
                self.float_x += speed * dx / dist
                self.float_y += speed * dy / dist
        else:
            dx, dy = dest[0] - self.float_x, dest[1] - self.float_y
            dist = math.hypot(dx, dy)
            if dist <= speed:
                self.float_x, self.float_y = dest
                self.destination = None
            else:
                self.float_x += speed * dx / dist
                self.float_y += speed * dy / dist
