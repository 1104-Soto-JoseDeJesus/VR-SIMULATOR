...
class Battlefield:
    ...
    def render_with_coords(self, armies: List["Army"]) -> str:
        ...
        This helper is intended for debugging and CLI visualisation where the
        default ``render`` method hides sub-hex movement by rounding to integer
        tiles.  Each army is displayed on its own line as ``Name@x,y`` with the
        ...
        for army in armies:
            if army.current_troop_count <= 0:
                continue
            lines.append(
                f"{army.name}@{army.float_x:.1f},{army.float_y:.1f}"
            )
        return "\n".join(lines)
