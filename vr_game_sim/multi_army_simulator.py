...
from .battlefield import Battlefield, step_towards, HEX_DIRECTIONS
        ...
        for army in self.armies:
            ...
            if army.destination:
                ...
                if occupant and occupant.team != army.team:
                    # ensure multiple attackers occupy different adjacent cells
                    stacked = any(
                        (
                            other is not army
                            and other.current_troop_count > 0
                            and other.x == army.x
                            and other.y == army.y
                        )
                    ...
                    if stacked:
                        slot = self._find_open_adjacent(occupant)
                        if slot:
                            army.x, army.y = slot
